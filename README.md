...
# How To Guide 
In order to make an agent that interacts with the enviornment, you must implement our StockAgent interface which will in return create a MarketSensor that will allow your agent to choose stocks that it might want to buy or sell. Then you'll need to implement a Portfolio and ProtfolioManager so that your agent has the ability to do the necessary calculations it would need to in order to decide which stocks to buy and which ones to sell. 
